...
class DemandeModel:
    ...
    def validate_demande(demande_id: int, valideur_id: int, action: str, 
        ...
        """Validate or reject a demande"""
        try:
            # Get demande info
            demande_data = db.execute_query('''
                SELECT d.user_id, d.nom_manifestation, d.status, d.type_demande,
            ...
            current_status = demande_data['status']
            
            if action == 'valider':
                if current_status == 'en_attente_dr':
                    # DR validates → goes to financier
                    DemandeModel.update_demande(
                        demande_id,
                        status='en_attente_financier',
                        valideur_dr_id=valideur_id,
                        date_validation_dr=datetime.now().isoformat(),
                        commentaire_dr=commentaire
                    )
                    return True, "Demande validée par le DR et envoyée à la validation financière."
                elif current_status == 'en_attente_financier':
                    # Combined Financier/DG validation
                    
                    # Get current validation state
                    demande_full_data = db.execute_query('''
                        SELECT valideur_financier_id, valideur_dg_id
                        FROM demandes
                        WHERE id = ?
                    ''', (demande_id,), fetch='one')

                    if not demande_full_data:
                         return False, "Demande non trouvée lors de la validation financière."

                    update_data = {}
                    message = ""
                    
                    valideur_role_data = db.execute_query('SELECT role FROM users WHERE id = ?', (valideur_id,), fetch='one')

                    if not valideur_role_data:
                         return False, "Rôle du valideur non trouvé."
                         
                    valideur_role = valideur_role_data['role']

                    # Update fields based on who is validating
                    if valideur_role == 'dr_financier':
                         update_data['valideur_financier_id'] = valideur_id
                         update_data['date_validation_financier'] = datetime.now().isoformat()
                         update_data['commentaire_financier'] = commentaire
                         message = "Demande validée par le Financier."
                    elif valideur_role == 'dg':
                         update_data['valideur_dg_id'] = valideur_id
                         update_data['date_validation_dg'] = datetime.now().isoformat()
                         update_data['commentaire_dg'] = commentaire
                         message = "Demande validée par le DG."
                    else:
                         # Ce cas ne devrait pas arriver si l'interface est correcte, mais par sécurité
                         return False, "Votre rôle ne vous permet pas de valider à cette étape."

                    # Check if both have validated AFTER applying the current validation
                    # We need to re-fetch or update the full data based on update_data
                    # A simpler way is to check if the *other* valideur field is already set
                    financier_already_validated = demande_full_data['valideur_financier_id'] is not None
                    dg_already_validated = demande_full_data['valideur_dg_id'] is not None

                    # Determine the new status
                    new_status = 'en_attente_financier' # Default to staying in this status

                    # La logique pour déterminer new_status en fonction de qui a validé et qui avait déjà validé
                    # est correcte en utilisant les IDs déjà présents + l'action actuelle
                    if valideur_role == 'dr_financier':
                         # Si le Financier valide, on regarde si le DG a déjà validé
                         if dg_already_validated:
                              new_status = 'validee'
                    elif valideur_role == 'dg':
                         # Si le DG valide, on regarde si le Financier a déjà validé
                         if financier_already_validated:
                              new_status = 'validee'

                    # Special case: if the same person validates twice (shouldn't happen with UI, but for robustness)
                    if valideur_role == 'dr_financier' and demande_full_data['valideur_financier_id'] == valideur_id:
                         return False, "Vous avez déjà validé cette demande."
                    if valideur_role == 'dg' and demande_full_data['valideur_dg_id'] == valideur_id:
                         return False, "Vous avez déjà validé cette demande."
                    ...
                    if new_status == 'validee':
                         message = "Demande validée et approuvée par le Financier et le DG."
                    # Si status est toujours en_attente_financier, le message initial de validation partielle est retourné
                ...
                if current_status == 'en_attente_dr':
                    ...
                    return True, "Demande rejetée par le DR."
                elif current_status == 'en_attente_financier':
                    # Get current validation state to record who rejected
                    demande_full_data = db.execute_query('''
                        SELECT valideur_financier_id, valideur_dg_id
                        FROM demandes
                        WHERE id = ?
                    ...
                    if not demande_full_data:
                         return False, "Demande non trouvée lors du rejet financier."
                         
                    update_data = {
                        'status': 'rejetee',
                    }
                    message = "Demande rejetée par les financiers."

                    valideur_role_data = db.execute_query('SELECT role FROM users WHERE id = ?', (valideur_id,), fetch='one')

                    if not valideur_role_data:
                         return False, "Rôle du valideur non trouvé lors du rejet."
                         
                    valideur_role = valideur_role_data['role']

                    # Record who rejected (utiliser valideur_role)
                    if valideur_role == 'dr_financier':
                         update_data['valideur_financier_id'] = valideur_id
                         update_data['date_validation_financier'] = datetime.now().isoformat()
                         update_data['commentaire_financier'] = commentaire
                         message = "Demande rejetée par le Financier."
                    elif valideur_role == 'dg':
                         update_data['valideur_dg_id'] = valideur_id
                         update_data['date_validation_dg'] = datetime.now().isoformat()
                         update_data['commentaire_dg'] = commentaire
                         ...
                         return False, "Votre rôle ne vous permet pas de rejeter à cette étape."
            ...
            # Si l'action n'est ni valider ni rejeter, ou si le statut n'est pas géré
            return False, "Action ou statut non géré pour cette demande."
