...
class WorkflowService:
    ...
    def validate_demande(demande_id: int, validator_id: int, action: str, 
        ...
        try:
            # Get demande and validator info
            demande = DemandeModel.get_demande_by_id(demande_id)
            if not demande:
                return False, "Demande non trouvée"
            
            validator = UserModel.get_user_by_id(validator_id)
            if not validator:
                return False, "Validateur non trouvé"
            
            # Check if validator has permission
            if not WorkflowService._can_validate(demande, validator):
                return False, "Vous n'êtes pas autorisé à valider cette demande"
            
            current_status = demande['status']
            
            if action == 'valider':
                success, message = WorkflowService._process_validation(
                    demande, validator, comment
                )
            elif action == 'rejeter':
                success, message = WorkflowService._process_rejection(
                    demande, validator, comment
                )
            else:
                return False, "Action non valide"
            
            if success:
                # Log activity
                ActivityLogModel.log_activity(
                    validator_id, demande_id, f'{action}_demande',
                    f"{action.capitalize()} demande {demande['nom_manifestation']}: {comment}"
                )
            
            return success, message
            
        except Exception as e:
            logger.error(f"Error validating demande: {e}")
            return False, f"Erreur: {e}"
